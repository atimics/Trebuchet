...
try {
  ...
  // Airdrop section (nested in the advanced panel, itself collapsed).
  try {
    await page.evaluate(() => { const d = document.getElementById('simpleAirdropDetails'); if (d) d.open = true; });
    await page.waitForTimeout(500);
    await shootEl('#simpleAirdropDetails', '04-airdrop-config.png');
    await page.evaluate(() => { const d = document.getElementById('simpleAirdropDetails'); if (d) d.open = false; });
  ...
  try {
    ...
    // Going back to simple discards the ladder we just enabled, which
    // raises the app's confirm dialog — itself a UI surface worth one
    // capture — then accept it to restore simple-mode defaults.
    await page.click('#returnToSimpleBtn');
    try {
      await page.waitForSelector('#genericConfirmModal.is-active', { timeout: 5_000 });
      await shootEl('#genericConfirmModal .modal-card', '06-confirm-dialog.png');
  ...
  // The inline launch-report preview — the feature the old GIF tried (and
  // failed) to demonstrate. Expand it and capture the report itself.
  try {
    await page.click('#step5ReportToggle');
    // The report renders into an iframe; give it a beat to paint.
    await page.waitForTimeout(1500);
    await shootCard(5, '11-launch-report-preview.png');
    await gifFrame('report preview');
    await page.click('#step5ReportToggle'); // collapse again for the next shot
    await page.waitForTimeout(400);
  } catch (e) {
    console.warn('report preview capture skipped:', e.message);
  }
  ...
  await page.locator('#launchSuccessModal .modal-card, #launchSuccessModal .modal-content').first()
    .screenshot({ path: join(outDir, '14-launch-success.png') });
  console.log('captured 14-launch-success.png');
